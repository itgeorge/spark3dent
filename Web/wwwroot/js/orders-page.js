...
if(window.S3DIcons) S3DIcons.hydrate(document);
    ...
    async function ordersBeforeLeave(from,to,navOptions={}){const allowed=await ordersDirtyGuard.beforeLeave(from,to,navOptions);if(allowed&&flowView&&from&&(from.name==='newOrder'||from.name==='editOrder')&&!flowView.isSafeTransition(from,to,navOptions))flowView.clearOrderFlowSession();return allowed}
    ...
    rootView=S3DOrders.RootView.create({api:ordersApi,getActor:()=>actor,loadClinics:loadClinics,openUiModal:openUiModal,closeUiModal:closeUiModal,onOpenOrder:code=>goOrderReview(code),onNewOrder:step=>goNewOrder(step),onShowShell:showAuthenticatedAppShell,onCloseFlowModals:()=>{if(flowView){flowView.closeDiscard();flowView.closeBeforeMinimum();flowView.clearOrderFlowSession()}else{closeDiscardOrderFlowPopup();closeBeforeMinimumConfirmPopup()}},onCloseCancelOrder:closeCancelOrderConfirmPopup,consumeRouteError:()=>{const msg=pendingOrdersRouteError;pendingOrdersRouteError='';return msg}});
