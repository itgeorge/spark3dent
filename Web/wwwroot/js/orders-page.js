...
if(window.S3DIcons) S3DIcons.hydrate(document);
    ...
    function goNewReservation(stepToOpen=1,opts){return ordersRouter.navigate(`reservation-new/${stepToOpen}`,opts)}
    function goReservationReview(id,opts){return ordersRouter.navigate(`reservation/${encodeURIComponent(id)}`,opts)}
    ...
    function goEditReservation(idToOpen,stepToOpen=1,opts){return ordersRouter.navigate(`reservation-edit/${encodeURIComponent(idToOpen)}/${stepToOpen}`,opts)}
    ...
    rootView=S3DOrders.RootView.create({api:ordersApi,getActor:()=>actor,loadClinics:loadClinics,openUiModal:openUiModal,closeUiModal:closeUiModal,onOpenOrder:code=>goOrderReview(code),onOpenReservation:id=>goReservationReview(id),onNewOrder:step=>goNewOrder(step),onNewReservation:step=>goNewReservation(step),onShowShell:showAuthenticatedAppShell,onCloseFlowModals:()=>{if(flowView){flowView.closeDiscard();flowView.closeBeforeMinimum()}else{closeDiscardOrderFlowPopup();closeBeforeMinimumConfirmPopup()}},onCloseCancelOrder:closeCancelOrderConfirmPopup,consumeRouteError:()=>{const msg=pendingOrdersRouteError;pendingOrdersRouteError='';return msg}});
    const reviewView=S3DOrders.ReviewView.create({api:ordersApi,getActor:()=>actor,showLogin:showLogin,showShell:showAuthenticatedAppShell,closeOrdersDay:()=>rootView&&rootView.closeOrdersDay(),openUiModal:openUiModal,closeUiModal:closeUiModal,replace:(path,opts)=>ordersRouter.replace(path,opts),navigate:(path,opts)=>ordersRouter.navigate(path,opts),onRouteError:msg=>{pendingOrdersRouteError=msg},onBack:opts=>goOrdersRoot(opts),onEdit:(code,step)=>goEditOrder(code,step),onEditReservation:(id,step)=>goEditReservation(id,step),clearFindHighlight:()=>rootView&&rootView.clearFindHighlight(),restoreFindHighlight:restart=>rootView&&rootView.restoreFindHighlightAfterReview(restart),onCancelled:async()=>{if(rootView){rootView.clearFindHighlight();await rootView.reload()}}});
    ...
    ordersRouter=HashRouter.createHashRouter({
      routes:[
        ...
        {name:'reservationReview',pattern:'reservation/:id',handler:ctx=>reviewView.showReservation(ctx.params.id)},
        {name:'newOrder',pattern:'new/:step',handler:ctx=>flowView.showNew(ctx.params.step)},
        {name:'newReservation',pattern:'reservation-new/:step',handler:ctx=>flowView.showNewReservation(ctx.params.step)},
        ...
        {name:'editReservation',pattern:'reservation-edit/:id/:step',handler:ctx=>flowView.showEditReservation(ctx.params.id,ctx.params.step)}
      ],
