(function(global){
  ...
  function create(options){
    ...
    function orderTeethLabel(o){return Format.orderWorkItems(o).map(i=>+i.toothStart===+i.toothEnd?String(i.toothStart):`${i.toothStart}-${i.toothEnd}`).join(', ')}
    function orderToothCount(o){const range=Format.orderTeethRange(o);if(range?.length)return range.length;return Format.orderWorkItems(o).length}
    function orderChipLabel(o){const teeth=orderTeethLabel(o);const prefix=`${Format.orderMaterialCalendarShort(o)} · ${teeth}`;return prefix.length>28?`${orderToothCount(o)} teeth · ${Format.orderMaterialCalendarShort(o)} · ${o.caseName||'—'}`:`${prefix} · ${o.caseName||'—'}`}
    ...
    function renderOrdersCalendarCell({cell,content,iso}){if(iso===ordersCalendarHighlightDate)cell.classList.add('orders-calendar-date-highlight');const dayOrders=ordersCalendarByDate.get(iso)||[];if(!dayOrders.length)return;cell.classList.add('orders-calendar-cell-has-orders');const openDay=()=>openOrdersDayPopup(iso,dayOrders);cell.onclick=e=>{if(e.target.closest('.orders-calendar-chip,.orders-calendar-more,.orders-calendar-count'))return;openDay()};content.appendChild(buildOrdersCalendarCountButton(dayOrders,openDay));dayOrders.slice(0,3).forEach(o=>{const chip=document.createElement('button');chip.type='button';chip.className='orders-calendar-chip';chip.textContent=orderChipLabel(o);const clinicName=Format.clinicDisplayNameForOrder(o,orderClinics);chip.title=`${o.caseName||''} ${clinicName}`.trim();if(actor()?.isLab)applyClinicAccent(chip,o);chip.onclick=e=>{e.stopPropagation();onOpenOrder(o.orderCode)};content.appendChild(chip)});if(dayOrders.length>3){content.appendChild(buildOrdersCalendarMoreButton(dayOrders,openDay))}}
    function openOrdersDayPopup(iso,dayOrders){ordersDayPopupTitle.textContent=Format.formatDateBulgarianWithWeekday(iso);ordersDayPopupSub.textContent=`${dayOrders.length} active ${dayOrders.length===1?'order':'orders'} • ${dayToothTotalText(dayOrders)}`;ordersDayPopupList.innerHTML='';for(const o of dayOrders){const row=document.createElement('button');row.type='button';row.className='orders-day-row';if(actor()?.isLab)applyClinicAccent(row,o);const clinicMeta=actor()?.isLab?esc(Format.clinicDisplayNameForOrder(o,orderClinics)):'';row.innerHTML=`<b>${esc(orderPopupPrimaryLabel(o))}</b><span>${esc(o.shortenedOrderCode||o.orderCode)} · ${esc(Format.shadeDisplay(o.shade))}${clinicMeta?` · ${clinicMeta}`:''}</span>`;row.onclick=()=>{closeOrdersDayPopup();onOpenOrder(o.orderCode)};ordersDayPopupList.appendChild(row)}openUiModal('ordersDay',ordersDayPopup,ordersDayPopupCloseBtn)}
