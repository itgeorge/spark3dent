(function(global){
  ...
  function create(options){
    ...
    const caseName = $('caseName'), extraNote = $('extraNote'), colorNote = $('colorNote'), shade = $('shade'), deadline = $('deadline'), impression = $('impression');
    const err = $('err'), codeEl = $('code'), deliveryCalendar = $('deliveryCalendar'), dateMsg = $('dateMsg');
    const overviewText = $('overviewText'), overviewShade = $('overviewShade'), overviewColorNote = $('overviewColorNote'), overviewDate = $('overviewDate'), overviewTeeth = $('overviewTeeth');
    const finalOverviewText = $('finalOverviewText'), finalOverviewShade = $('finalOverviewShade'), finalColorNote = $('finalColorNote'), finalOverviewDate = $('finalOverviewDate'), finalOverviewTeeth = $('finalOverviewTeeth'), finalCaseName = $('finalCaseName'), finalExtraNote = $('finalExtraNote');
    ...
    let calendarMonth = new Date(new Date().getFullYear(), new Date().getMonth(), 1), calendarRequest = 0, deliveryPickSkips = 0, deliveryCalendarInstance = null, deliveryMinimumDate = '', pendingBeforeMinimumDate = '', deliveryCalendarByDate = new Map(), deliveryCalendarClinics = {};
    ...
    function productCategory(){return material==='pmma'||material==='pmmaTelio'?'temporary':'permanent'}
    ...
    function renderOverviewInto(textEl,shadeEl,colorNoteEl,dateEl,teethEl){const range=selectedTeethRange(),previewItems=selectedWorkItemsForPreview();setOverviewField(textEl,overviewOrderBaseText());setOverviewShade(shadeEl,overviewShadeLine());setOverviewColorNote(colorNoteEl,colorNote?.value||'');setOverviewField(dateEl,deadline.value||selectedDate?formatDateBulgarianWithWeekday(deadline.value||selectedDate):'');syncOverviewBodyLayout(teethEl?.closest('.overview-body'),range);renderSelectedTeethPreview(teethEl,range,previewItems)}
    function renderOverview(){renderOverviewInto(overviewText,overviewShade,overviewColorNote,overviewDate,overviewTeeth)}
    function renderFinalOverview(){renderOverviewInto(finalOverviewText,finalOverviewShade,finalColorNote,finalOverviewDate,finalOverviewTeeth);setOverviewField(finalCaseName,caseName.value.trim());const note=extraNote.value.trim();if(finalExtraNote){setOverviewField(finalExtraNote,note?`Note: ${note}`:'');finalExtraNote.classList.toggle('hidden',!note)}}
    ...
    function draft(delivery=deadline.value||selectedDate||'',start=impression.value,end='2026-07-05'){const d={caseName:caseName.value,impressionDate:impression.value,productCategory:productCategory(),material,workItems:workItemsPayload(),requestedDeliveryDate:delivery,shade:shade.value,notes:extraNote.value,colorNote:colorNote?.value||'',start,end};if(editMode&&editingOrder?.orderCode)d.orderCode=editingOrder.orderCode;if(actor()?.isLab&&!editMode&&targetClinic.value)d.clinicCode=targetClinic.value;return d}
    ...
    function dateUnavailableReason(status){if(status?.isSelectable)return '';return status?.reason||'Unavailable'}
    ...
    function renderDeliveryDateCell(ctx,byDate){const status=byDate.get(ctx.iso)||{date:ctx.iso,isSelectable:false,reason:'Unavailable'};S3DOrders.DeliveryDatePicker.renderDateCell(ctx,{status,selectedIso:deadline.value,impressionIso:impression.value,isLabOverride:isLabBeforeMinimumOverride,dayOrders:deliveryCalendarByDate.get(ctx.iso)||[],orderClinics:deliveryCalendarClinics,isLab:!!actor()?.isLab,orderClicksEnabled:false,onOpenDay:options.openOrdersDay,onSelect:requestDeliveryDateSelection,bindReason:(cell,statusOrText)=>typeof statusOrText==='string'?bindDateCellPop(cell,statusOrText):bindDateReasonPop(cell,statusOrText)})}
    ...
    async function loadDates(){if(!deliveryCalendar)return;hideDateReasonPop();syncImpressionToday();const request=++calendarRequest;const month=calendarMonth,bounds=MonthCalendar.bounds(month),startIso=toIsoDate(bounds.start),endIso=toIsoDate(bounds.end);const [availabilityResult,calendarResult]=await Promise.all([ordersApi.dateAvailability(draft(deadline.value||selectedDate||startIso,startIso,endIso)),ordersApi.calendarOrders(startIso,endIso)]);const j=availabilityResult.data;if(request!==calendarRequest)return;if(!availabilityResult.ok){dateMsg.textContent=j.error;dateMsg.className='msg err';return}if(calendarResult.ok){deliveryCalendarClinics=calendarResult.data.clinics||{};deliveryCalendarByDate=new Map((calendarResult.data.days||[]).map(d=>[d.date,d.orders||[]]))}else{deliveryCalendarClinics={};deliveryCalendarByDate=new Map()}deliveryMinimumDate=j.minimumDate||'';const changed=syncDeliverySelection(j.dates);const picked=deadline.value||selectedDate;if(changed&&picked){deliveryPickSkips=0;const want=monthForIso(picked);if(want.getTime()!==calendarMonth.getTime()){calendarMonth=want;return loadDates()}}else if(!firstSelectableDate(j.dates)&&deliveryPickSkips<18){deliveryPickSkips++;calendarMonth=new Date(calendarMonth.getFullYear(),calendarMonth.getMonth()+1,1);return loadDates()}deliveryPickSkips=0;dateMsg.textContent='';dateMsg.className='msg hidden';updateSelectedDeliveryDisplay();hideDateReasonPop();const byDate=new Map(j.dates.map(d=>[d.date,d]));const nonWorkingDays=new Set(j.dates.filter(d=>d.isClosed).map(d=>d.date));const options={month,nonWorkingDays,renderCell:ctx=>renderDeliveryDateCell(ctx,byDate),onMonthChange:m=>{calendarMonth=m;if(step===3)loadDates()}};if(!deliveryCalendarInstance)deliveryCalendarInstance=MonthCalendar.create(deliveryCalendar,options);else deliveryCalendarInstance.setOptions(options)}
