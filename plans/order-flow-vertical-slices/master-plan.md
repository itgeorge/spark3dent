...
- 2026-06-08 Slices 13-16: `dotnet build Web/Web.csproj --no-restore -p:UseSharedCompilation=false` passed; `dotnet build Cli/Cli.csproj --no-restore -p:UseSharedCompilation=false` passed; `dotnet test Orders.Tests/Orders.Tests.csproj --no-restore -p:UseSharedCompilation=false` passed (66 tests); `dotnet test Database.Tests/Database.Tests.csproj --no-restore -p:UseSharedCompilation=false` passed (85 tests after adding bootstrap clinic-code collision coverage); `dotnet test Web.Tests/Web.Tests.csproj --no-restore -p:UseSharedCompilation=false` passed (106 tests); full `dotnet test --no-restore -p:UseSharedCompilation=false` passed (Configuration 10, Storage 41, Orders 66, Accounting 61, Database 84, Invoices 251, Web 106); `node --check` on extracted `Web/wwwroot/iam.html` inline script passed; CLI temp-DB smoke passed for bootstrap success, refusal without `--reset`, and reset success. Headless Chromium browser smoke passed against a temp DB on `http://127.0.0.1:61288`: CLI-bootstrapped lab custom-secret login, IAM client prefill, prompt-based clinic create/edit, member add/rotate/deactivate/reactivate, rotated member login, clinic deactivate, and deactivated clinic login rejection.
...
- `GET /api/scheduling/orders/find?code=&limit=` (actor-scoped find by full code or unambiguous shortened code; returns `order`, `listPage`, `listModeRecommended`, `reason`)
...
- `Orders/SchedulingAuthService.cs`
- `Orders/Repositories.cs`
- new/reworked lab, clinic organization, and member domain/repository types
- `Database/Entities/SchedulingAuthSessionEntity.cs`
- new lab/clinic/member DB entities and repositories
- `Database/AppDbContext.cs`
- `Database/Migrations/*`
...
- `Web/Api.cs`
- `Web/WebProgram.cs`
- audit contracts/entities if actor organization fields are added
- tests in `Orders.Tests` / `Database.Tests` / `Web.Tests`
...
- `Web/wwwroot/orders.html`
- `Web/wwwroot/js/app-chrome.js`
- audit API/CLI output if terminology changes
- tests and docs/plans
...
- `Web/Web.csproj`
- `Web/WebProgram.cs`
- new `Web/IamApi.cs` or equivalent
- `Web/wwwroot/js/app-chrome.js`
- IAM CSS/JS if split out
- lab/clinic/member repositories from Slice 10
